...
## Only runs if predesigned_guides is defined in config.yaml
if "predesigned_guides" in config:
    rule combine_predesigned_guides:
        input:
            regions = lambda wildcards: [
                config["predesigned_guides"][guide_set]["regions"]
                for guide_set in config["predesigned_guides"]
            ],
...
rule filter_guides:
    ...
    shell:
        ...
        cat {input.combined_guides} | grep -v "TTTT" | awk '{{if ($5 > 50) print $1 "\t" $2 "\t" $3 "\t
        """
